...
**第一轮 —— 救片。** 18 张被规则栈硬性剔除、被 M3 保留的照片,owner
**17 张同意 M3**。M3 很擅长救回检测器误杀的帧 —— 它拿到了闭眼计数和
清晰度方差,仍然选择推翻它们。

**第二轮 —— 其余分歧。** 从剩下的分歧里抽 40 张,其中风险最高的 19 张
全部取走。结果按方向分得非常干净:

| M3 想做的事 | 它判对了 |
|---|---|
| 把规则判死的救上来(`cull →` keep/maybe) | **7 / 7** |
| 把 `maybe` 降为 `cull` | **3 / 3** |
| 把 `keep` 软化成 `maybe` | 6 / 11 |
| **把规则想留的片砍掉** | **8 / 19** |

**M3 救片可靠,砍片不可靠。** 在唯一会毁掉照片的那个动作上 —— 砍掉一张
留片 —— 它比抛硬币还差。所以证据只买到一项权力,`--vlm-authority rescue`
给的就是这一项:**可以推翻硬性剔除,其余一概无权干涉。**

### 这还不能证明什么

两轮抽的都是**两个系统有分歧的行**,等于只在规则栈最吃力的地方考它。
在这个样本上 M3 看着远好于实际;而在全量 408 行上,87% 的标签仍是规则栈
自己的判决,模型改动任何一行都自动记错。**两个数字都定不了默认值** ——
一个偏袒规则,一个偏袒模型,`pixcull m3 eval` 现在会直说,而不是把好看的
那个印出来。

`pixcull m3 review --only random` 生成能定论的那种样本:不看两边是否一致
随机抽行。在它被标注完之前,`vlm_authority` 默认保持 `off`。
