...
## 最近更新
- **v2.99**:`pixcull deliver` 写出你真正要交出去的那个文件夹。原来的导出只能
  给 XMP sidecar 或评分 CSV —— 对 Lightroom 有用,对一个客户就坐在旁边等着的
  人没用,于是影楼流程要么绕回另一个软件,要么手工拷文件。

  手工拷贝恰好在最要紧的时刻毁掉 PixCull 独有的两件事:这一帧属于拍摄的哪一段,
  以及哪几张是同一个瞬间、其中哪张最好。第二件正是和客户对话的全部内容。这个
  文件夹把两者都留住了。
- **v2.98**:经微信转发之后,唯一还能认出照片的标识只剩像素本身。写在说明里的
  文件名,在相册重排、客户截了一部分、或者两条发送失败的瞬间就没了;位置不是
  标识,位置恰恰是会变的那个东西。

  所以序号被烧进画面 —— 左上角,约为画面高度的 9%,不透明底板上的白字。按微信
  的典型处理链路实测(长边压到 1080、q=50 重编码、聊天里显示约 220 px 缩略图)
  仍然可读,并且有测试断言经过这一整轮之后底板依然是实心的。
- **v2.97**:客户在场模式。动手之前先在一个 5069 张的 run 上量了一屏:539 处
  判断性文字,每张卡都写着「保留」和一个分数,顶部还有一行"多少张被标记为删除"。

  这套流程本来就是客户坐在摄影师旁边、看同一块屏。于是客户读到的是机器对自己
  婚礼的判决,每张还挂着一个数字。这会把"挑照片"变成"为照片辩护"。Shift+C 把
  这些全部隐藏,同时保留文件名,开机器的人照样能干活。
- **v2.96**:上一轮留下的未决项,查下来根本不是回归。两次机器负载被当成同一次
  来比较了,差异来自机器,不来自代码。
- **v2.95**:运行摘要报出来的,是判官已经推翻掉的决定。`Keep=6 Maybe=0 Cull=0`
  就打印在「判官改动了 6 个决定」的下一行,而 CSV 里每一行都写着 `cull`。

  两个数据源,其中一个是旧的:决定是打分过程中逐行追加进列表的,而当 VLM 权威
  设为 primary 时,判官会在之后就地改写它们。摘要读的是列表,CSV 是从数据框导出
  的。摄影师读到"全部保留",打开结果发现一张都没留。
- **v2.94**:唯一能产出真正盲标的工具,写出来的格式恰好是溯源护栏拒收的那一种。
  `pixcull m3 label` 呈现一张照片、一个序号和两个按钮;护栏要的是一个 `source`
  字段、每张照片一条记录,而这张标注表写的是一条记录带一个判决映射。

  两边互不知情,于是那个能解锁三项测量的标注工作,产出了一份被读成 `unknown`
  并被拒收的文件 —— 宣称存在却不可达,恰好发生在这个项目下一个真实数字本该
  产生的地方。
