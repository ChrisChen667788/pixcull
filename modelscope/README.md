...
- **v3.80**:Lightroom 导入器打不开 Lightroom 目录。所有测试都自建 SQLite 夹具、
  用的是默认日志模式;而真实目录是 WAL,只读连接打不开。十个测试全程通过。现在先
  拷贝再读拷贝,摄影师自己的文件永远不会被以写入方式打开。

- **v3.79**:「不设上限」不等于「无界」。测量规划器只对自己的上限设防,而每日 API
  额度会一次一个调用地拦下来 —— 于是它存在的全部意义(避免跑到一半停住)恰恰能靠
  「不设任何上限」触发,而且是静默的。
...
6. **连拍峰值排序,输的那些回到 `maybe`** —— 亚秒级连拍组自动选峰值帧;
   同组里没选上的帧降为 `maybe`(v3.83),出现在你面前而不是混在精选里。
   不会判 `cull`:第二个表情往往才是你要的那张
