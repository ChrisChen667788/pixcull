...
## ③ Selects 模式(⌘1)整个失效 —— 已修(本轮后续)

核验后发现比预想更糟:不止 `window.render()` 死 no-op,**keep+maybe 过滤从未接进
`render()`**(注释承诺「treat decision==='selects' as keep+maybe」+「CSS 类做视觉
提示」,**两者都没实现**)。结果 ⌘1 此前只弹个 toast,网格纹丝不动。

修:① `render()` 过滤逻辑特判 `decision === "selects"` → 只留 keep+maybe;②
`_toggle()` 改用词法可见的 `render()`(非死 `window.render()`)+ 同步 decision 药丸
(进入时全灭、退出复位);③ 补 `#grid.selects-mode` 的 brass 顶线提示(box-shadow,
不影响布局),兑现原注释承诺。

验证(无头,临时标 1 张 cull):进入 Selects → 网格 7→6(恰好隐藏 cull)、`.selects-mode`
类生效、药丸全灭;退出 → 6→7 复原、药丸回「全部」;零 JS 错误。门禁全绿。
