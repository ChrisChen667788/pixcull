...
# v2.31 — packaged-install support. Until now every asset path was
# `Path(__file__).parent.parent / …`, i.e. "this file lives in
# <repo>/scripts/", which is true for a git clone and FALSE for a
# pip-installed wheel (where serve_demo ships inside the pixcull package
# and there is no repo root). Two resolvers split the concerns:
#
#   _pkg_root()  — the installed `pixcull` package dir. Always correct;
#                  templates + locale ship inside the wheel, so anything
#                  REQUIRED at runtime resolves through here.
#   _repo_root() — the git checkout root, or None when pip-installed.
#                  Only for dev/optional assets (samples/, models/,
#                  docs/, training CSV dumps) that the wheel omits; call
#                  sites already degrade when they're missing.
def _pkg_root() -> Path:
    """Directory of the installed `pixcull` package (works both from a
...
def _repo_root() -> Path | None:
    ...
    file's dir — a pip install has no such layout."""
    # This module ships INSIDE the package (pixcull/report/serve_app.py),
    # so the checkout root is three levels up — and only exists in a clone.
    cand = Path(__file__).resolve().parent.parent.parent
    return cand if (cand / "pyproject.toml").is_file() else None


# v2.5-P0-1 — pure serialization/coercion helpers extracted to an
# importable, unit-tested module (first slice of de-monolithing this file).
# Imported back into this namespace so every existing call site is unchanged.
from pixcull.report.serve_util import (  # noqa: E402,F401
    _clean_csv_string,
    _f,
    _html_escape,
    _opt_int,
...
_RESULTS_HTML_PATH = (
    _pkg_root() / "report" / "templates" / "results.html"
)
...
# _scrub_nan / _safe_dumps → pixcull.report.serve_util (v2.5-P0-1),
...
# _html_escape → pixcull.report.serve_util (v2.5-P0-1).
...
def _render_video_review_html(rid: str) -> str:
    ...
    """
    return _read_template("video_review.html").replace("__RUN_ID__", rid)


# _VIDEO_REVIEW_HTML extracted to templates/video_review.html (v2.5-P0-1 slice 2);
# loaded lazily via _read_template() in its render function.
...
def _render_timeline_html(rid: str) -> str:
    ...
    return _read_template("timeline.html").replace("__RUN_ID__", rid)


# _TIMELINE_HTML extracted to templates/timeline.html (v2.5-P0-1 slice 2);
# loaded lazily via _read_template() in its render function.
...
# pixcull.report.serve_util (v2.5-P0-1), imported at the top of this file.
...
def _retrain_in_background(include_auto: bool, also_goldenset: bool) -> None:
    ...
    try:
        ...
        import sys as _sys
        _rr_scripts = _repo_root()
        scripts_dir = (_rr_scripts / "scripts") if _rr_scripts else Path(__file__).parent.resolve()
        ...
        repo_root = _repo_root() or (Path.home() / ".pixcull")
...
class _Handler(BaseHTTPRequestHandler):
    ...
    def _serve_disagreement_page(self) -> None:
        ...
        # v2.28 — static shell extracted to templates/pages/disagreement.html;
        # the row count + the two computed tables splice in at placeholders.
        # Byte-identical to the prior inline build (route-diff verified).
        html_body = (
            _read_template("pages/disagreement.html")
            .replace("<!--__DIS_NROWS_META__-->", f"{report.n_total_rows:,} 条记录 · ")
            .replace("<!--__DIS_ROWS__-->", rows_html)
...
(function() {{
  ...
  form.addEventListener("submit", async e => {{
    ...
    def _serve_healthz(self) -> None:
        ...
        _rr_models = _repo_root()
        models_dir = (_rr_models / "models") if _rr_models else Path("/nonexistent")
    ...
    def _serve_history_page(self) -> None:
        ...
        # v2.28 — extracted to templates/pages/history.html. The static
        # shell (+ shared design-tokens CSS) is a template; the two runtime
        # injections — the run count and the run cards — splice in at
        # placeholders. Byte-identical to the prior inline build (route-diff).
        page = (
            _read_template("pages/history.html")
            .replace("/*__DESIGN_TOKENS_CSS__*/", _DESIGN_TOKENS_CSS)
            .replace("<!--__HISTORY_COUNT__-->", f"{len(entries)} 个 run")
            .replace("<!--__HISTORY_CARDS__-->", cards_html)
        )
    ...
    def _serve_tether_page(self) -> None:
        ...
        rest of the product.
        """
        # v2.28 — extracted to templates/pages/tether.html (fully static
        # shell; only the shared design-tokens CSS is spliced in, same as
        # the v2.16 page extractions). Byte-identical to the prior inline
        # r"""...""" + _DESIGN_TOKENS_CSS build (route-diff verified).
        page = _read_template("pages/tether.html").replace(
            "/*__DESIGN_TOKENS_CSS__*/", _DESIGN_TOKENS_CSS)
        body = page.encode("utf-8")
        self.send_response(200)
        self.send_header("Content-Type", "text/html; charset=utf-8")
...
# bootstrap loader.
# v2.16-P0 — extracted to templates/pages/first_run.html (217 lines of inline HTML);
# loaded via the same _read_template as video_review/timeline (v2.5).
_FIRST_RUN_HTML = _read_template("pages/first_run.html")
...
# v2.16-P0 — extracted to templates/pages/privacy.html (109 lines of inline HTML);
# loaded via the same _read_template as video_review/timeline (v2.5).
_PRIVACY_HTML = _read_template("pages/privacy.html")
...
# v2.16-P0 — extracted to templates/pages/verticals.html (1850 lines of inline HTML);
# loaded via the same _read_template as video_review/timeline (v2.5).
_VERTICALS_HTML = _read_template("pages/verticals.html")
...
# loaded via the same _read_template as video_review/timeline (v2.5).
_UPLOAD_HTML = _read_template("pages/upload.html").replace(
    "/*__DESIGN_TOKENS_CSS__*/", _DESIGN_TOKENS_CSS)
...
# v2.16-P0 — extracted to templates/pages/admin.html (662 lines of inline HTML);
# loaded via the same _read_template as video_review/timeline (v2.5).
_ADMIN_HTML = _read_template("pages/admin.html").replace(
    "/*__DESIGN_TOKENS_CSS__*/", _DESIGN_TOKENS_CSS)
...
# v2.16-P0 — extracted to templates/pages/admin_perf.html (476 lines of inline HTML);
# loaded via the same _read_template as video_review/timeline (v2.5).
_ADMIN_PERF_HTML = _read_template("pages/admin_perf.html").replace(
    "/*__DESIGN_TOKENS_CSS__*/", _DESIGN_TOKENS_CSS)
