...
## 最近更新
- **v3.87**:三张视频截图进了抓图脚本,顺手挖出两件比图本身值钱的事。

  照片+视频时间线一直在读错文件。`pixcull run` 把 `scores.csv` 写在
  `output/` 下,`pixcull video` 写在 run 根目录,而这一页只看过根目录 ——
  于是这张以「照片」命名的页面,对每一个正常的照片 run 都一张照片都不显示。
  它唯一读得到的那种布局是视频 run,而循环上方那行注释说它会跳过视频抽帧,
  它从来没跳过:34 帧的片子回来变成 34 张「没有拍摄时间的照片」,计进表头,
  合计成「共 35 个时间点」,而实际只有一个。相邻照片还会被并成一行,不管
  时间差多远,所以横跨一年的一批照片会渲染成同一个瞬间。

  演示片的人脸护栏,能证明的比它听起来少。它打印的是「containment: 0 detected
  face(s) fell outside the frosted region」,被当成了「没有人脸漏出来」。620 帧
  里有 145 帧检测器什么都没找到,没东西可包含,于是这些帧直接通过。第 101 帧
  它找到了两个框,都落在主体的羽绒服上,没有一个在她脸上 —— 那张侧脸没被遮、
  完全可辨 —— 包含率 100% 通过。两个误检就把这道检查完整地满足了,而本机两个
  模型都根本看不见那张脸。现在护栏只说它真正知道的事;能被拍进截图的帧,改由
  一份「人已在全尺寸下看过」的清单按内容哈希放行。三张截图改用一段完全不需要
  遮挡的素材重拍。

- **v3.86**:27 张截图里有 15 张没有任何东西会重新生成。其中两张进了抓图脚本,
  下次 UI 变动会连它们一起重拍。`20-scenes-navigator` 拍了两次:第一次只是把
  元素滚进视野,拍到的是网格 —— 因为导航条默认隐藏、要点开关才填充。**在正确的
  图注下拍错了东西**,正是这份台账存在的理由。

- **v3.85**:撤回 v3.83。同样 149 张的盲标结果:被降级的 100 张里,摄影师留下了
  87 张 —— 一百张塞进复核区,为了抓那十三张。排名保留并照常展示,去掉的只是让它
  影响判决。一条测试会让任何「读 `is_burst_peak` 又写 `decision`」的函数失败,
  不管叫什么名字,并写明重试需要什么新证据。

- **v3.84**:同一个文件夹跑出了不同的判决 —— `Keep=97 Maybe=53`,然后
  `Keep=95 Maybe=55`,中间没有改过代码。EXIF 时间只到秒而连拍每秒好几张,并列
  是常态;并列时保留并行打分的完成顺序,而算法只比较相邻行,落在并列边界的那一
  行决定两组合不合并。
...
- **v3.82**:一段连续的 149 帧,工具留下了全部 149 张。它已经找出 28 个近重复簇
  (覆盖其中 127 帧),并标出 99 张不是各自连拍里最好的那张 —— 然后把它们也留下了。

  判决是一帧一帧做完、并在工具知道哪些帧是同组之前就定稿的,所以连拍排名晚了
  十三行,来不及起作用。v3.82 只是让这次运行把话说出来 —— 一场什么都没删的跑分,
  不该读起来像一场「每张都是精选」。
...
(上图为真机跑一段实拍雪橇素材、聚焦 lightbox + 时间轴的实页。18 / 19 / 23
这三张从 v3.87 起由 `scripts/brand/capture_video_shots.py` 重拍,落盘前先验
它确实拍到了说明文字里那件事。)
头部 🎨 调色下拉(v2.0-P2-2)一键套用胶片预置(Fuji Eterna / Kodak
Vision3 / Arri 709A / Teal-Orange / B&W),主画面 + 每个候选缩略图实时
参数化预览(仅预览,不改原片)。

![视频审片 · 🎨 调色预览 — 整段套用 Kodak / Arri / Teal-Orange / B&W LUT,主画面和每一张候选缩略图同时改(此处 B&W)](docs/screenshots/19-video-grade.png)
**照片 + 视频同一条时间线(`/timeline/<run_id>`)** —— 一次拍摄里的照片与视频片段
按拍摄时间排在一起,视频卡片显示时长 · 帧数 · 候选数 · 卖点标签,一键跳进审片台。

![照片 + 视频时间线 — 同一条轴上按分钟分组的照片,末尾接着两段视频卡片](docs/screenshots/23-video-timeline.png)
...
6. **连拍峰值排序** —— 亚秒级连拍组自动选峰值帧;**只报告,不据此判决**
   (v3.83 曾降级,v3.85 撤回:149 张盲标里被降级的 100 张,摄影师会留 87 张)
