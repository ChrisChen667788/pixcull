...
- **v3.81**:评分体系量的每一样,都分不开这位摄影师的留和删
- **v3.80**:目录读取器读不了真实的目录
- **v3.79**:「不设上限」不等于「没有边界」
- **v3.78**:归因后端不是没做完,是做错了形状
- **v3.77**:演示片段换成真实素材,人脸用毛玻璃遮掉而不是避开
- **v3.76**:三处展示的东西已经不再为真
- **v3.75**:产品告诉你它会做、却没做的四件事
- **v3.74**:那条「安装冒烟」流水线在排练一场没人会做的安装
- **v3.73**:Phase A.1,而它本要扩展的那个文件本身是错的
- **v3.72**:那些没迁移的字面量是浅色主题的 bug,不是装饰性欠债
- **v3.71**:迁移计数器在数唯一一件没人能迁移的东西
- **v3.70**:那些永远只在太晚时才跑的检查,以及一次有洞的扫描
- **v3.69**:一个决定照片上不上传的设置,什么都没做
- **v3.68**:那道一致性门禁只写了一个包的名字,于是只守住了一个包
- **v3.67**:README 教人跑一条从来不存在的命令
- **v3.66**:三件标记为「需要人拍板」的事,其中两个问题本身就问错了
- **v3.65**:抓「说明过期」的那道门禁,钉死在一个已经结束的版本号系列上
- **v3.64**:封面图里混进了一张能认出脸的照片;numpy 的上限理由已经过期
- **v3.63**:换成真照片,以及那个一直返回 500 的示例数据按钮
