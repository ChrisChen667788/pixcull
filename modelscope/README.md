...
## 最近更新
- **v2.45**:17 条 CLI 命令里只有 4 条有链路测试,v2.42–v2.44.3 加的整块视频
  能力一条都没有。四条新测试驱动 `video → cut --render`,断言成片比源片短(等长
  说明编辑被忽略、只是整片重编码),以及删光时拒绝出片而不是写出零长文件。两次
  变异都重现了历史症状。CI 补装 ffmpeg —— 没有它这四条会 skip,整轮报绿却什么
  都没测。
- **v2.44.3**:说话人标签在适配器里写了三个版本、一次没跑过。它确实能出标签,
  但所有段都是 0 号,换声学差异极大的素材、显式告知人数都没用。根因在 FunASR 的
  聚类器:少于 20 个嵌入就硬返回"所有人都是 0 号",在看嵌入之前。59 秒的对话越过
  这道门槛,两人分离、18 条标对 18 条。门槛之下,"只有一个人"和"太短分不出"的输出
  完全一样,所以两种情况现在都报 `speaker=None`。
- **v2.44.2**:`pixcull cut --render`,审片页也有"出片"按钮。默认硬切,不用
  reel 那半秒溶解 —— 按文字剪保留下来的常是同一句话的两半,溶解会从两侧吃掉时间,
  正好吃掉你选择保留的字。验收方式是把成片重新过一遍 ASR:被删的字在音轨里确实
  没了,保留的完好。
- **v2.44.1**:按文字剪进了浏览器(见下方截图)。划掉一行,或选中行内几个字,
  画面跟着一起没。按词选只在引擎给了每字真实时间时才提供 —— 插值出来的时间会把
  切点放到错误的帧上。另外修好 `video_review.html` 里 32 处从未生效的 CSS 变量:
  这个页面用自己的调色板,而 v2.43 的转录面板照抄了共享 token,hover 背景一直是空的。
- **v2.44**:通用模型把「长焦」听成「掌交」、「备选」听成「被选」,一个字错整条
  字幕就废。88 条领域词表按先验写死,不是照着测试集的错误往回补;在从未见过的
  10 句留出集上,CER 从 2.59% 降到 1.11%。编辑是叠在不可变转录上的删除区间,
  所以撤销不可能让文字和时间轴对不上。
