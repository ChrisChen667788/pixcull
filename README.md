...
**v3.86** — Fifteen of the twenty-seven committed screenshots were
regenerated by nothing. Two are in the capture script now, so the next UI
change re-shoots them. `20-scenes-navigator` took two attempts: the first
scrolled the element into view and photographed the grid, because the
...
照片一样 Keep / Cull。**上图是真机跑一段实拍雪橇素材渲染的实页(聚焦 lightbox + 时间轴)。**
这三张(18 / 19 / 23)从 v3.87 起由 `scripts/brand/capture_video_shots.py`
重拍,每张在落盘前先验它确实拍到了说明文字里那件事。
