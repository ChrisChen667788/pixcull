...
## 最近更新
- **v3.68**:用户可以据以安装的依赖清单有两份,而且有一道门禁保证两份不会
  说不同的话。它把 `numpy` 写成了字面量,于是十六个共享依赖里,它只守住了那个
  已经被咬过一次的。三个漂移了 —— `torch`、`torchvision`、`transformers`
  在这边都有大版本天花板,在托管 Studio 的依赖清单里一个都没有。

  这三个不是随机的三个:它们恰好是天花板用疼痛换来的那三个。凡是痛到需要
  加天花板的依赖,恰好就是公开 demo 一直没加天花板在跑的依赖 —— 访客既看不到、
  也钉不住、也改不了的那个安装,保护是最少的。

- **v3.67**:README 教人跑一条 `deliver` 命令。这条命令从来不存在;真正发布的
  那条叫 `pixcull view-folder`。它十二个版本前被写进快速上手,跟着上了 PyPI,
  而专门为抓这种事建的门禁看不见它 —— 那道门禁只读一个文件的一个小节,而这句
  话在三节之外。现在每份文档里每一条 `pixcull …` 都会拿去和 CLI 自己的命令树
  比对,于是又抓出第二条:`export` 上一个同样不存在的参数。

- **v3.66**:有三个问题被标记为「需要人来拍板」。其中两个,问题本身就问错了。
  配色不是三套竞争品牌色里三选一 —— 那三个文件装的是产品强调色、字标渐变、
  和一份过期拷贝,是三种角色,不是三种意见。仓库的重量也不在那 9 MB 样张上,
  而在旁边 62 MB 的截图里,现在是 31 MB,在 README 显示的尺寸上看不出差别。

- **v3.65**:v3.53 建来抓「最近更新过期」的那道门禁,读发布提交用的正则
  钉死在它写下时正在用的版本号系列上。下一个大版本一到,它就哑了,并且一路
  哑过六十四个版本,而它旁边每一条测试都是绿的。一道钉死在今天数据形状上的
  护栏,有一个它自己不会宣布的过期时间。

- **v3.64**:封面图里混进了一张能认出脸的照片。样张里有一帧,画面右缘有个小女孩
  跑过田野;在 1600 px 的工作尺寸上她只有三十像素高、看着就是个小点,而在原始
  分辨率下她的脸完全清晰可辨。这一帧当时在主视觉里,也在四张截图里。

  值得写下来的是另一半:产品自己的人脸检测器把整个候选池都报成了干净的。一个
  单发检测器拿到一张缩小过的 5472 px 画面,它要找的东西在送进去的图里只有十像素
  高。所以筛查现在靠眼睛,检测器只当指路的。认真看一遍,还从同一个「干净」池子
  里翻出两张人像,和一块能读出来的车牌。

  同一版把 numpy 提到 `>=2.0,<2.5`。旧的 `<2` 上限有两条理由,重新实测后都不成立
  了;真正的上限来自 numba。这个 pin 还有另外两份副本没跟着改,其中一份是运行时
  警告,会对着一个装对了的环境喊「你该降版本」。
- **v3.63**:样例数据原本是六张渐变图,上面烧着「Mountain Range」和「Eagle in
  Flight」的字样 —— 访客点「示例数据」看到的就是这个。现在换成三十二张真照片。
  而那个按钮本身一直在对所有人返回 500:它要把 `samples/output` 拷进一次运行,
  而 `.gitignore` 里那条光秃秃的 `output/` 会匹配任意层级下同名的目录,于是这个
  目录根本提交不进去。
