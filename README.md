...
**v3.76** — Three places showed something that had stopped being true. The
popover for undecided frames fired on a hard-coded score band that sits
entirely inside the cull zone, so it appeared on decided frames and never on
undecided ones. Corrections made in the review page did not reach the XMP
export: the pipeline said cull, the photographer said keep, and Lightroom was
handed cull. And fifteen of twenty-seven screenshots are regenerated by
nothing, which is now written down per image with a date.

**v3.75** — Declining to upload your photographs did not stick. The prompt
said "staying on-device for this and every future run" and wrote nothing, so
the next run asked again. Also: a run served from a different directory
reported every photo as missing, and a keyboard shortcut advertised in three
places had no handler.
