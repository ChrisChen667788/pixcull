...
- **v3.81**:追查首批盲标得出的 73% 一致率:摄影师删掉而工具留下的那些帧,在每个
  维度上都**高于**双方都认可的那些。构图 +0.20、瞬间 +0.49、清晰度 +555。所以这
  不是阈值太松,而且再多的修正数据也补不上一个评分体系里没有的维度。

  更糟的是,个性化路径把这种「没有信号」变成了一个强烈的意见 —— 六个轴里四个指向
  相反,夹到零之后,全部权重落在两个零点一星的差上。负的分离度是反向证据,不是证据
  缺失。
