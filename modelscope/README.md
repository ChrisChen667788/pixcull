...
- **v3.63**:样例数据原本是六张渐变图,上面烧着「Mountain Range」和「Eagle in
  ...
  目录根本提交不进去。
- **v3.62**:README 的主视觉原来是一张手绘的产品界面 SVG,里面每一张「照片」都是
  一个空的灰色方块。一个照片筛选工具,开篇第一张图里没有照片,两秒钟的测试就已经
  输了。
- **v3.61**:`pip install -e ".[sync]"` 关掉了缺口台账的最后一行,而关掉它才看见
  底下的洞。有三条测试标着 `@pytest.mark.slow`;工作流里两处 pytest 调用都写着
  `-m "not slow"`,却没有任何一条流水线写 `-m slow`,于是它们是被取消选中、而不是
  被跳过,在 CI 里一次都没跑过。其中一条是镜头切分检测,耗时 0.29 秒。「慢」已经
  不再表示慢,而变成了一个东西被扔进去的地方。
- **v3.60**:`pip install pixcull` 会连带装上 pytest、ruff 和 yapf:十个包、
  15.1 MB 别人的开发工具链,经由 pyiqa 进来。保留了 —— pyiqa 提供审美这一个轴,
  为省这点体积把它去掉,等于对所有升级的人静悄悄砍掉一个打分轴。查这件事的时候
  发现了属于我们的那一半:`import pyiqa` 本来就是惰性的,但没有任何东西接住它失败
  的情况,于是一个没装 pyiqa 的环境丢掉的不是一个轴,是整次运行。
