...
## 最近更新
- **v3.72**:浅色主题一直在该画深青铜的地方画香槟金。六十二处颜色字面量写在
  两个主题都会跑的规则里,于是深色主题的硬编码值跑到了白纸上;已发布的浅色
  主题截图里有 769 个香槟色像素。用渲染验证:深色主题逐像素不变,浅色主题变了,
  而这个差别就是修复本身。

  另有十一个 `var()` 引用指向从未定义过的属性,渲染的永远是它们的 fallback ——
  其中三个是紫色配色时代的遗留,v3.66 的重命名够不着,因为那一遍看的是定义,
  而这些是指向不存在之物的引用。

- **v3.71**:统计这项工作的那个计数器,一直在描述没人能做的事。它把
  `--accent: #d5b584` 也算进去了 —— 那是 token 被*定义*的地方,不可能变成指向
  自己的引用;而且它只读 `<style>` 里面,那是单文件页面上色的三个地方之一。
  在其中一个模板里那就是全部:数到三条,三条全是定义,而七条真实用法藏在
  JavaScript 拼出来的 SVG 里,它一条也看不见。

- **v3.70**:前二十个版本里有八个需要补一次提交,最近几次全是同一个形状:
  改动是对的,围绕它的记账没跟上,CI 在推送九分钟后才说。`make preflight`
  把这些检查压到十八秒。隐私扫描也有个洞 —— 十六个文本后缀的白名单,意味着
  五十八个已跟踪文件从来没被打开过。里面没有泄漏;但一个从不打开某类文件却
  报告仓库干净的扫描,是在宣称它没有检查过的事。

- **v3.69**:`PIXCULL_VLM_MODE=off` 什么也没做。决定照片会不会为云端判图上传的
  那个开关只有命令行参数,而 `PIXCULL_VLM_MODEL`、`PIXCULL_VLM_API_KEY`、
  `PIXCULL_VLM_WORKERS` 都存在 —— 于是这个名字既是最自然的猜法,又和一个真名
  只差一个字母。现在它是真的了,而且任何产品不认识的 `PIXCULL_*` 名字都会在
  启动时报出来,附最接近的真名。
