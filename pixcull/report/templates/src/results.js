...
@@MODULE:00-run-tag.js@@
  ...
  function _applyLangToDom() {
    document.querySelectorAll("[data-i18n]").forEach(el => {
      ...
      // v2.69 — replace the LABEL, keep the furniture.
      //
      // `el.textContent = txt` deletes every child, and the decision
      // pills now carry a `.pill-n` count. Translating the panel erased
      // the numbers it had just been given. It happened to look fine
      // because buildDynamicFilters() runs after this and puts them
      // back — the counts were surviving on call order, which is not a
      // property anything was checking.
  ...
  async function _applyLang(lang) {
    ...
    try {
      ...
      // v2.69 — and the scene / style chips, which are built from row
      // data through _t() at init, i.e. BEFORE this fetch resolves.
      // Without this they keep whatever _t() returned against an empty
      // dictionary: the raw enum. Every `scene.*` string added in this
      // version would have shipped unreachable — the exact defect this
      // version exists to remove, reintroduced by its own fix.
  ...
  document.addEventListener("DOMContentLoaded", () => {
    ...
    // v2.69 — zh_CN fetches the dictionary too.
    //
    // The old fast path skipped it, reasoning that "the HTML is rendered
    // server-side in zh". That holds for static markup and fails for
    // everything built by JS: a `_t()` call has no server-rendered
    // fallback to be correct, only whatever string the caller happened
    // to type. It failed for static markup as well, wherever the
    // hardcoded fallback was not actually Chinese — the decision pills
    // read `keep` / `maybe` / `cull` to the primary audience for as long
    // as that panel has existed, while `decision.keep = 保留` sat in
    // zh_CN.json unread.
    //
    // 192 strings over loopback. The saving was never worth an
    // assumption that nothing checked.
    _applyLang(stored);
  ...
  const stats = [
    ...
    `<span class="maybe">${_t("decision.maybe", "待定")} `
      + `<b data-stat="maybe">${summary.n_maybe}</b></span>`,
    `<span class="cull">${_t("decision.cull", "剔除")} `
      + `<b data-stat="cull">${summary.n_cull}</b></span>`,
  ...
  function buildDynamicFilters() {
    ...
    // v2.69 — the chips said `landscape`, `documentary`, `stilllife` to a
    // Chinese photographer: internal enum values rendered raw, in the one
    // panel they use to navigate a 5,000-frame shoot. `scene.*` had zero
    // entries in every one of the 13 locales, so there was nothing to
    // render but the identifier.
    //
    // `unknown` sorts last regardless of count and carries its own class.
    // It is not a twelfth kind of photograph — it is the classifier
    // saying it could not tell, and listing it between `abstract` and
    // `fashion` invites filtering by it as though it described the
    // picture.
    // v2.69 — counts on the decision pills.
    //
    // The panel is where you choose what to look at, and it could not
    // tell you how many there were: the numbers lived in a separate
    // strip at the top of the window, so "how much of this shoot is
    // still undecided" needed two glances and a mental join. Every other
    // group in this panel already carries its counts.
    (() => {
      const by = {all: rows.length, keep: 0, maybe: 0, cull: 0};
      rows.forEach(r => { if (by[r.decision] !== undefined) by[r.decision] += 1; });
      document.querySelectorAll("#decisionPills .pill[data-d]").forEach(el => {
        const n = by[el.dataset.d];
        if (n === undefined) return;
        let c = el.querySelector(".pill-n");
        if (!c) {
    ...
    const _sceneRank = ([s, n]) => (s === "unknown" ? -1 : n);
    const sceneEl = document.getElementById("sceneFilters");
    sceneEl.innerHTML = Object.entries(sceneCounts)
      .sort((a, b) => _sceneRank(b) - _sceneRank(a))
      .map(([s, n]) => {
        const unk = s === "unknown";
        const tip = unk
          ? _t("filter.scene_unknown_hint", "场景分类器没能判定的照片") : "";
        return `<span class="pill${unk ? " pill-unknown" : ""}"`
          + ` data-scene="${esc(s)}"${tip ? ` title="${esc(tip)}"` : ""}>`
          + `${esc(_t("scene." + s, s))} <span style="opacity:0.5">${n}</span></span>`;
      })
      .join("");
    const styleEl = document.getElementById("styleFilters");
    styleEl.innerHTML = Object.entries(styleCounts)
      .sort((a, b) => b[1] - a[1])
      .map(([s, n]) => `<span class="pill" data-style="${esc(s)}">`
        + `${esc(_t("style." + s, s))} <span style="opacity:0.5">${n}</span></span>`)
      .join("");
...
@@MODULE:03-calm-toolbar.js@@
  ...
  function render() {
    ...
    function renderCard(r) {
      // v2.86 — ask for what this screen actually needs.
      //
      // The grid displays a thumbnail at ~278 CSS px. On a Retina
      // display that is 556 device pixels, and the route served 420
      // whatever ?w= asked for, so a photographer judging focus was
      // looking at an upscaled image in a tool built to decide whether
      // frames are sharp. At DPR 1 the same fixed 420 is 1.5x more than
      // is drawn.
      //
      // Two buckets only. Every existing cached thumbnail is a 420, and
      // a bucket per client width would multiply the cache by the number
      // of screen sizes rather than by two.
      const thumb = `/thumb/${run_id}/${encodeURIComponent(r.filename)}`
        + ((window.devicePixelRatio || 1) > 1.5 ? "?w=840" : "");
    ...
    if (!html) {
      ...
      // v2.68.2 — `h` is measured by the CALLER, before any mutation.
      // Reading it here forced a layout inside the loop that was also
      // removing cards, and this grid always has ~5,069 direct children
      // (the de-materialiser swaps their contents, never their number),
      // so each forced layout cost 8-10ms. A batch of 50 paid for 50.
      const _dematerialize = (card, h) => {
        const idx = parseInt(card.dataset.segIdx || "-1", 10);
        if (idx < 0) return;
        const ph = document.createElement("div");
        ph.className = "card-placeholder";
        ph.style.height = (h || PLACEHOLDER_HEIGHT) + "px";
      ...
      _dematIo = new IntersectionObserver((entries) => {
        ...
        for (const ent of entries) {
          if (!ent.isIntersecting && ent.target.classList.contains("card")) {
            doomed.push(ent.target);
          }
        }
        if (!doomed.length) return;
        // Phase 1: read every height. Phase 2: mutate. Interleaving them
        // made this the only source of jank on the page — measured with
        // the observer disconnected, a 7s scroll produced ZERO long
        // tasks; connected, the same scroll blocked for 388ms.
        const heights = doomed.map(c => c.offsetHeight || PLACEHOLDER_HEIGHT);
        for (let i = 0; i < doomed.length; i++) {
          _dematerialize(doomed[i], heights[i]);
        }
  ...
  function openCompare(clusterKey) {
    ...
    cmpBody.querySelectorAll(".cmp-best-btn").forEach(btn => {
      btn.addEventListener("click", async () => {
        ...
        // v3.9 — record the COMPARISON, not only the N labels it
        // implies. "I preferred this frame to those, and they were
        // nearly identical" is a much stronger statement than "this one
        // is good and those are bad", and it is the shape preference
        // learning consumes. Its own file: annotations.jsonl is
        // latest-wins on the whole record.
        try {
          await fetch(`/pairwise/${run_id}`, {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({
              winner: pickFn,
              losers: members.filter(m => m.filename !== pickFn)
                             .map(m => m.filename),
              cluster: clusterKey,
              context: "burst",
            }),
          });
        } catch (_e) { /* the labels below still land */ }
          ...
          try {
            await fetch(`/annotation/${run_id}/${encodeURIComponent(m.filename)}`, {
              ...
              body: JSON.stringify({
                ...
                // v3.9 — a sibling rejected here is not a photograph the
                // photographer disliked; it lost to a near-identical
                // frame. Marked so the taste profile can exclude it
                // instead of averaging near-identical rows into both
                // its keep and its cull bucket.
                source: (lbl === "keep") ? "compare_winner" : "compare_rejected",
              }),
            });
...
@@MODULE:28-a11y-toggle.js@@
@@MODULE:29-theme-toggle.js@@
  // ==================================================================
