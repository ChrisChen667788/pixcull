...
每张照片的 AI 分数都解释自己 —— 热力图 / 六轴星级 / 逐张读解 —— 你永远知道它为什么这么判。

**云端判官是否开启,取决于你这台机器上有没有 MiniMax key,不是一个固定的默认值**:

| 你的机器 | 一条裸 `pixcull run` |
|---|---|
| 没有 MiniMax key | **全程本机,什么都不外发** |
| 有 key、从没授权过 | 先问一次;拒绝、或任何非交互式运行,都留在本机 |
| 有 key **且**授权已记录 | **会上传** —— 运行时会打印出来 |
