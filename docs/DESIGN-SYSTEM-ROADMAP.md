...
> **实测(2026-09-10,v3.67 顺手量的)—— 照这条字面执行会是一次回归。**
> 在模板源码里扫一遍,总共 694 个非 ASCII 符号,但它们是两类东西:
>
> | | 数量 | 是什么 |
> |---|---|---|
> | 彩色 emoji | **190**(47 种,15 个文件) | 这一条真正说的对象:`🪣` `📡` `🏆` `🎯` `👍` … |
> | 排版符号 | **504**(23 种) | `→` ×271、`★` ×59、`✓` ×38、`←` ×30、`✕` ×29 —— 用页面字体渲染,跨平台一致 |
>
> "把 emoji 全部替换成图标"如果按字面做,会把后面那 504 个也换掉,
> 而它们本来就是正确的排版。这一条的范围是前面那 190 个。
>
> **另外:几乎没有一个 emoji 是"承重"的。** 扫遍所有 `<button>` / `<a>`,
> 只有一个控件的可见标签只有 emoji(`results.src.html` 的 `✨` 巡览按钮),
> 而它有 `title`。`results.js` 里那 65 处全部是"emoji + 真实文字"的前缀
> 形式(`🎯 每连拍峰值`、`🎬 时序场景`)。唯一真正只靠 emoji 传达信息的
> 地方是人脸缩图加载失败时的 `onerror="this.outerHTML='👤'"` —— 那一处
> 是可修的正确性问题,其余是审美问题。
>
> 所以这一条仍然值得做,但它是**审美改动**,需要一个 before/after 才说得清
> 值不值,不是一个能靠测量收尾的缺陷修复。
