...
**2. No client-selection workflow. — CLOSED 2026-09-02, in a narrower shape than proposed here.** For Chinese commercial studios in the wedding, portrait, and travel-shoot market, the step between the photographer's internal cull and the client's own photo selection is a major operational workflow. Xuanpian, PixCake, Baidu Netdisk AI, Xiangsuben, and Evoto Instant all address it, and when this document was written PixCull had no answer. The owner then declined the gallery-platform shape this section originally proposed, and specified the studio's existing motion instead: the photographer exports to an iPad or Mac and the client reviews on-site, or watermarked frames go over WeChat and feedback comes back remotely. PixCull now optimises inside that motion rather than replacing it — 客户在场模式 (v2.97), a burned-in index that survives a WeChat re-encode plus reply parsing (v2.98), the viewable folder export (v2.99), and client picks kept in their own file with their own filter (v3.0–v3.0.2). What remains open is the return leg — reconciling returned picks against the photographer's own decisions — which is `ROADMAP-v3.1-v3.27-charter.md` §v3.27.
...
> **Superseded.** The owner declined the QR-linked web-gallery shape below and
> chose to optimise inside the studio's existing on-site / WeChat review motion.
> What shipped instead — v2.97 through v3.0.2 — is described in §gap-2 above.
> The proposal is kept unedited for the record of what was proposed and why the
> narrower call was better informed.
