...
**v3.81** — Chasing the 73% agreement from the first blind corrections: the
frames the photographer culled and the tool kept score *higher* than the ones
both agreed on, on every axis. Composition +0.20, moment +0.49, sharpness
+555. So it is not a loose threshold, and no amount of correction data fixes
a dimension the rubric does not have.

Worse, the personalisation path turned that absence into a strong opinion —
four of six axes pointed the wrong way, clamping dropped them, and the whole
weight landed on two gaps of a tenth of a star. A negative gap is evidence
against, not missing evidence.

**v3.80** — The Lightroom importer could not open a Lightroom catalogue.
Every test built its own SQLite fixture in the default journal mode; real
catalogues are in WAL, and a read-only connection cannot open one. Ten tests
passed throughout. It copies first and reads the copy now, so the
photographer's own file is never opened for writing.

**v3.79** — "No ceiling" was not the same as "unbounded". The measurement
planner refused only against its own ceiling, while a daily API cap stopped
calls one at a time as they happened — so the halfway stop it exists to
prevent was reachable by setting no ceiling at all, silently.

**v3.78** — The per-axis attribution heatmap had a real backend, tests, and
no way to reach it. Wiring it up started with measuring it, and it turned out
to produce a byte-identical picture for all six axes: the per-axis models moved
into the package two dozen versions ago and this consumer never followed, so
every axis fell back to the same generic map.

It could not have worked anyway. The axis scorers are trees over 29 named
measurements — horizon tilt, face count, subject fraction — and never see
pixels. A saliency map over a neural network explains a different model than
the one that made the decision. In a tool whose point is telling you why, a
confident wrong explanation is worse than none, so it is gone rather than
fixed. Ten tests covered it and all passed; none compared two axes.

**v3.77** — The screenshot for edit-by-text was an ffmpeg test pattern with
synthesised speech, and read as a rendering fault. It is real footage with its
own audio now, with the faces frosted — speech runs across almost the whole
