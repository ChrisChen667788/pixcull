...
> **2026-08-12 复核。** 我上一轮报「四个版本全部落地」**说过头了** ——
> 模块都造了,但有 5 项没接线,产品里够不到。已于 `7400238` 补完。
> 门禁 **1921 passed / 9 skipped / 0 failed**。
>
> | 版本 | 状态 |
> |---|---|
> | v2.48 P0/P1/P2 | ✅ 全部 |
> | v2.49 | ✅ 工具 6/6 · ❌ **测量本身未跑**(需真实 key) |
> | v2.50 | ✅ 5/6 · ⏸ **ModelScope 同步 + 截图重摄**(需 push,已约定等 eval) |
> | v2.51 | ✅ 6/6(⑥ 于 v2.52.1 补接) |
> | v2.52 | ✅ 6/6(③④⑤⑥ 于 v2.52.1 补接) |
>
> **两项未完成,都不是我能单方面做完的:**
>
> 1. **v2.49⑥ `docs/M3-EVAL.md` 不存在** —— 报告由 eval 生成,eval 需要真实
>    MiniMax key 调 M3,我不代收代管密钥。一条命令:
>    `bash scripts/m3_first_run.sh`
> 2. **v2.50⑥ GitHub ⇄ ModelScope 同步 + 截图重摄** —— 需要 push,而 push
>    是发布公开承诺的实质变更。已与 owner 约定:**先跑完 eval 再推**。
>
> **报告的结论允许是「不该翻默认值」。** 若 M3 并不更准,v2.50 的默认值应
> 改回 `off`,并把改写过的 47 处文案一并改回 ——
...
2. **时延与用量实测** — 608 张的真实 token 消耗、wall-clock、缓存命中率。
   把 charter 里 `~15 分钟` 的估算换成实测数字。
