...
#: A cluster of one has a trivial peak — the frame is its own best, and
#: demoting nothing is the right answer.
MIN_CLUSTER_FOR_DEMOTION = 2

#: What the reason column says, so the review page can explain the
#: verdict rather than just showing it.
DEMOTION_REASON = "连拍非峰值 — 同组里有更好的一张"


def demote_non_peak_bursts(df):
    """v3.83 — a frame the product itself ranked below its burst sibling
    stops being a `keep` and becomes a `maybe`.

    The measurement that prompted this: 149 contiguous frames from one
    shoot went in and 149 keeps came out. Twenty-eight near-duplicate
    clusters, the largest holding thirty frames of the same ridge, and
    `is_burst_peak` was False on ninety-nine of them. The product had
    already worked out which frame won each burst and then handed the
    photographer every loser as a keep.

    It could not have done otherwise. `decide()` runs per frame inside a
    loop, and `is_burst_peak` is a cross-frame fact that does not exist
    on the dataframe until thirteen lines later. Every frame was judged
    alone, and a burst is by definition not one frame.

    So this is a post-pass, which is the only place the fact exists.

    Deliberately `maybe`, not `cull`. A loser in one burst is often what
    the photographer wanted — a second expression, a guest reacting —
    and the product does not get to throw those away on a similarity
    score. `maybe` puts them in front of the person instead of hiding
    them, which is the difference between culling and deciding.

    Only ever demotes. A `cull` stays culled, and nothing is promoted:
    winning your burst is not evidence of anything on its own.
    """
    if df is None or len(df) == 0:
        return df
    for col in ("decision", "is_burst_peak", "cluster_id"):
        if col not in df.columns:
            return df

    sizes = df["cluster_id"].value_counts()

    def _in_real_cluster(cid) -> bool:
        try:
            if cid is None or str(cid).strip() in ("", "-1", "nan"):
                return False
        except Exception:       # noqa: BLE001 — any odd cell shape
            return False
        return int(sizes.get(cid, 0)) >= MIN_CLUSTER_FOR_DEMOTION

    def _is_non_peak(v) -> bool:
        # The column arrives as a real bool from rank_burst_peaks and as
        # the string "False" after a CSV round trip. Both mean the same
        # thing and both have to demote, or the behaviour depends on
        # whether the frame has been through a file.
        if isinstance(v, str):
    ...
    changed = 0
    for i in df.index:
        if str(df.at[i, "decision"]).strip() != "keep":
            continue
        if not _is_non_peak(df.at[i, "is_burst_peak"]):
            continue
        if not _in_real_cluster(df.at[i, "cluster_id"]):
            continue
        df.at[i, "decision"] = "maybe"
        if "reason" in df.columns:
            prev = str(df.at[i, "reason"] or "").strip()
            df.at[i, "reason"] = f"{prev}; {DEMOTION_REASON}" if prev else DEMOTION_REASON
        changed += 1
    return df
